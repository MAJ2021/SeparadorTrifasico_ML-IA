model DosSeparadoresTrifasicosRealistaPerturbaciones_V42
  ...
  Modelica.Blocks.Sources.CombiTimeTable perturbOil(
    table=[0, Qin_oil_base;
           3600, Qin_oil_base;
           7200, Qin_oil_base*5.0;
           10800, Qin_oil_base;
           14400, Qin_oil_base*0.05;
           18000, Qin_oil_base;
           24000, Qin_oil_base],
...
equation
  ...
  elseif time >= 18000 and time < 20000 then
    Qin_oil = 1.5 * valveOilIn * perturbOil.y[1];
  elseif time >= 20000 and time < 24000 then
    Qin_oil = valveOilIn * perturbOil.y[1] * (1 + 0.2*sin(time/300));
  else
  ...
  if (time >= 10800 and time < 14400) or (time >= 18000) then
    Qout_oil = Qin_oil * (1 + osc*0.1);   // salida ≈ entrada en meseta
  elseif (time >= 14400 and time < 18000) then
...
end DosSeparadoresTrifasicosRealistaPerturbaciones_V42;
