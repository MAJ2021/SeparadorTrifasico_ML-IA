model DosSeparadorsParaleloDesarroloV49_28_15
  ...
  Modelica.Blocks.Continuous.PI pidSep1Petroleo(k=2.0, T=60);
  Modelica.Blocks.Continuous.PI pidSep2General(k=1.0, T=180);
  Modelica.Blocks.Continuous.PI pidSep2Petroleo(k=2.0, T=60);

  // Coeficientes
  parameter Real Kv_agua = 3.5;
  parameter Real Kv_petroleo = 6.0;
  parameter Real Kseg = 10.0;
  parameter Real beta = 0.7;
  parameter Real beta_oil = 1.0;
...
equation
  ...
  sep1.Q_out_petroleo = Kv_petroleo * max(0, pidSep1Petroleo.y) * (sep1.nivelPetroleo/V_sep)
                        * (1 + beta_oil*(sep1.nivelPetroleo_pct - SP_petroleo)/SP_petroleo);
  ...
  sep2.Q_out_petroleo = Kv_petroleo * max(0, pidSep2Petroleo.y) * (sep2.nivelPetroleo/V_sep)
                        * (1 + beta_oil*(sep2.nivelPetroleo_pct - SP_petroleo)/SP_petroleo);

end DosSeparadorsParaleloDesarroloV49_28_15;
