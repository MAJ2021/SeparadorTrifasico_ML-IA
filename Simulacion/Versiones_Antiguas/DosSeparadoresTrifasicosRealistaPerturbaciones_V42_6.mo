model DosSeparadoresTrifasicosRealistaPerturbaciones_V42_6
...
equation
  ...
  if (time >= 10800 and time < 12600) then
    Qin_oil = valveOilIn * Qin_oil_base * 1.2;   // entrada moderada en meseta intermedia
  elseif (time >= 12600 and time < 14400) then
  ...
  // Oscilaciones pseudo-aleatorias suavizadas
  osc = 0.03*sin(time/137) + 0.02*sin(time/271) + 0.01*sin(time/59);

  // Salida regulada por PID con oscilaciones
  Qout_oil = valveOilOut * (V_oil/V_sep) * Qin_oil_base * (1 + osc);
...
end DosSeparadoresTrifasicosRealistaPerturbaciones_V42_6;
