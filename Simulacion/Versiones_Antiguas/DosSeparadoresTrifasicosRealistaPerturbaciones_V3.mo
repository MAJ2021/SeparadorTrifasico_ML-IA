within ;
model DosSeparadoresTrifasicosRealistaPerturbaciones_V3
...
equation
  ...
  // Válvulas reescaladas (factor 0.1 en lugar de 0.01)
  Qout_oil1   = min(max(0, pidOil1.y   * 0.1), Qin_oil1);
  Qout_water1 = min(max(0, pidWater1.y * 0.1), Qin_water1);
  Qout_gas1   = min(max(0, pidGas1.y   * 0.1), Qin_gas1);
  ...
  Qout_oil2   = min(max(0, pidOil2.y   * 0.1), Qin_oil2);
  Qout_water2 = min(max(0, pidWater2.y * 0.1), Qin_water2);
  Qout_gas2   = min(max(0, pidGas2.y   * 0.1), Qin_gas2);




end DosSeparadoresTrifasicosRealistaPerturbaciones_V3;
