model DosSeparadoresParaleloDesarrolloV49_28_18
  ...
  Modelica.Blocks.Continuous.PI pidSep1Petroleo(k=3.5, T=30);
  Modelica.Blocks.Continuous.PI pidSep2General(k=1.0, T=180);
  Modelica.Blocks.Continuous.PI pidSep2Petroleo(k=3.5, T=30);
  ...
  parameter Real gamma = 0.15;   // refuerzo cuadrático
  parameter Real delta = 0.8;    // refuerzo lineal
...
equation
  ...
  sep1.Q_out_petroleo = Kv_petroleo * max(0, pidSep1Petroleo.y) * (sep1.nivelPetroleo/V_sep)
                        + delta * max(0, sep1.nivelPetroleo_pct - SP_petroleo)
  ...
  sep2.Q_out_petroleo = Kv_petroleo * max(0, pidSep2Petroleo.y) * (sep2.nivelPetroleo/V_sep)
                        + delta * max(0, sep2.nivelPetroleo_pct - SP_petroleo)
                        + gamma * (max(0, sep2.nivelPetroleo_pct - SP_petroleo))^2;

end DosSeparadoresParaleloDesarrolloV49_28_18;
