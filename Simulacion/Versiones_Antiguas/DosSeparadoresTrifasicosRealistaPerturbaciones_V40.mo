model DosSeparadoresTrifasicosRealistaPerturbaciones_V40
  inner Modelica.Fluid.System system;

  parameter Real V_sep   = 17.0 "Volumen del separador m3";
  parameter Real Qin_oil_base = 456/86400 "Caudal base de aceite m3/s";
  parameter Real tauRec = 500 "Constante de tiempo de recuperación (más rápida)";
  ...
  Modelica.Blocks.Sources.CombiTimeTable perturbOil(
    table=[0, Qin_oil_base;
           ...
           10800, Qin_oil_base;
           14400, Qin_oil_base*0.05;   // déficit
           18000, Qin_oil_base;
           24000, Qin_oil_base],
...
equation
  // Balance con recuperación suavizada (no se anula nunca)
  der(V_oil) = (Qin_oil - Qout_oil)
               - (V_oil - spDynamic)/(tauRec*(1 + 50*exp(-abs(V_oil - spDynamic))));
  ...
  elseif time >= 18000 and time < 20000 then
    Qin_oil = 2.0 * valveOilIn * perturbOil.y[1];   // boost recuperación
  elseif time >= 20000 and time < 24000 then
  ...
  // Lógica de salida
  if (time >= 10800 and time < 14400) or (time >= 18000) then
    // Mesetas: salida ≈ entrada para estabilizar en 54%
    Qout_oil = Qin_oil * (1 + osc) + valveOilOut*Qin_oil_base;
  elseif (time >= 14400 and time < 18000) then
    // Déficit
...
end DosSeparadoresTrifasicosRealistaPerturbaciones_V40;
