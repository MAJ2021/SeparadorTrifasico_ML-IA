model DosSeparadorsParaleloDesarroloV49_28_12
 parameter Real V_sep = 17;
  ...
  Modelica.Blocks.Continuous.PI pidSep1Petroleo(k=1.5, T=150);
  Modelica.Blocks.Continuous.PI pidSep2General(k=0.8, T=200);
  Modelica.Blocks.Continuous.PI pidSep2Petroleo(k=1.5, T=150);

  // Coeficientes de válvula y seguridad
  parameter Real Kv_agua = 2.0;
  parameter Real Kv_petroleo = 2.0;
  parameter Real Kseg = 5.0;

  model Separador
    parameter Real V = 17;
    parameter Real Kseg_sep = 5.0;
  ...
  equation
    ...
    // Caudal de seguridad: actúa si el nivel supera 90%
    Q_seguridad = Kseg_sep * max(0, (nivelGeneral_pct - 90));
  end Separador;
