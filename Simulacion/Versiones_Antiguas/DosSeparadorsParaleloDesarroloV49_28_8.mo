model DosSeparadorsParaleloDesarroloV49_28_8
  ...
  equation
    // Balance dinámico con saturación
    der(nivelAgua)     = (Q_in_agua - Q_out_agua)/3600;
    der(nivelPetroleo) = (Q_in_petroleo - Q_out_petroleo)/3600;

    nivelGeneral      = max(0, nivelAgua + nivelPetroleo);

    // Escalado correcto en %
    nivelAgua_pct     = max(0, (nivelAgua/V)*100);
    nivelPetroleo_pct = max(0, (nivelPetroleo/V)*100);
    nivelGeneral_pct  = max(0, (nivelGeneral/V)*100);
...
equation
  ...
  // Caudales de salida dependientes del nivel
...
end DosSeparadorsParaleloDesarroloV49_28_8;
