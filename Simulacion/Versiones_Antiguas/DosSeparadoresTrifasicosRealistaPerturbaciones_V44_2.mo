model DosSeparadoresTrifasicosRealistaPerturbaciones_V44_2
  ...
  Modelica.Blocks.Continuous.PID pidOilOut(k=0.01, Ti=5000, Td=0.1);
  Modelica.Blocks.Continuous.PID pidOilIn(k=0.01, Ti=5000, Td=0.1);
...
equation
  // Balance: recuperación desactivada en meseta intermedia
  ...
  // Entrada
  if (time >= 10800 and time < 12600) then
    Qin_oil = valveOilIn * Qin_oil_base;   // entrada estable en meseta
  elseif (time >= 12600 and time < 14400) then
  ...
  // Salida
  if (time >= 10800 and time < 12600) then
    Qout_oil = Qin_oil * (1 + osc*0.05);   // salida ≈ entrada en meseta
  else
...
end DosSeparadoresTrifasicosRealistaPerturbaciones_V44_2;
