model DosSeparadoresTrifasicosRealistaPerturbaciones_V41
  inner Modelica.Fluid.System system;

  parameter Real V_sep   = 17.0 "Volumen del separador m3";
  parameter Real Qin_oil_base = 456/86400 "Caudal base de aceite m3/s";
  parameter Real tauRec = 500 "Constante de tiempo de recuperación (rápida)";
...
equation
  // Balance con recuperación suavizada (se atenúa cerca del setpoint)
  ...
  if (time >= 10800 and time < 14400) or (time >= 18000) then
    ...
    Qout_oil = Qin_oil * (1 + osc*0.2) + 0.5*valveOilOut*Qin_oil_base;
  elseif (time >= 14400 and time < 18000) then
    // Déficit
...
end DosSeparadoresTrifasicosRealistaPerturbaciones_V41;
