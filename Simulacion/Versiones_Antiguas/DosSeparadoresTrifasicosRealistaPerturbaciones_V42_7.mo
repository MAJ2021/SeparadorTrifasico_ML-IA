model DosSeparadoresTrifasicosRealistaPerturbaciones_V42_7
  inner Modelica.Fluid.System system;

  parameter Real V_sep   = 17.0 "Volumen del separador m3";
  parameter Real Qin_oil_base = 456/86400 "Caudal base de aceite m3/s";
  parameter Real tauRec = 1000 "Constante de tiempo de recuperación";
...
equation
  // Balance con recuperación activa
  ...
  if (time >= 10800 and time < 12600) then
    Qin_oil = valveOilIn * Qin_oil_base;   // entrada estable en meseta intermedia
  elseif (time >= 12600 and time < 14400) then
  ...
  // Oscilaciones pseudo-aleatorias moderadas
  osc = 0.02*sin(time/137) + 0.015*sin(time/271) + 0.01*sin(time/59);

  // Salida
  if (time >= 10800 and time < 12600) then
    // Meseta intermedia: salida limitada ≈ entrada
    Qout_oil = min(Qin_oil, (V_oil/V_sep)*Qin_oil_base) * (1 + osc);
  elseif (time >= 12600 and time < 14400) then
    // Déficit
...
end DosSeparadoresTrifasicosRealistaPerturbaciones_V42_7;
