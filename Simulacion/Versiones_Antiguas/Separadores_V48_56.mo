...
equation
  ...
  Qout_total1 = min(apertura_total1*Cv_total*sqrt(Pgas1/1e5)*nivel_total1/100, max(V_agua1,0));
  Qout_oil1   = min(apertura_oil1*Cv_oil*sqrt(Pgas1/1e5)*nivel_oil1/100, max(V_oil1,0));
  ...
  Qout_total2 = min(apertura_total2*Cv_total*sqrt(Pgas2/1e5)*nivel_total2/100, max(V_agua2,0));
  Qout_oil2   = min(apertura_oil2*Cv_oil*sqrt(Pgas2/1e5)*nivel_oil2/100, max(V_oil2,0));
  Qout_gas2   = max((Pgas2-1e5)/1e5*0.05,0);
end Separadores_V48_56;
