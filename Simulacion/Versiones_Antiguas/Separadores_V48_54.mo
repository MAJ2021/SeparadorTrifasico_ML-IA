model Separadores_V48_54
  ...
  // Estados dinámicos
  Real V_agua1(start=5), V_oil1(start=2), Pgas1(start=3.5e5);
  Real V_agua2(start=5), V_oil2(start=2), Pgas2(start=3.5e5);
  ...
  Real apertura_total1(start=0), apertura_oil1(start=0);
  Real apertura_total2(start=0), apertura_oil2(start=0);
...
equation
  ...
  // Anti-windup: integral solo acumula si apertura < 1
  der(integral_total1) = if apertura_total1 < 1 then error_total1 else 0;
  der(integral_oil1)   = if apertura_oil1   < 1 then error_oil1   else 0;

  apertura_total1 = min(max(Kp*error_total1 + Ki*integral_total1,0),1);
  apertura_oil1   = min(max(Kp*error_oil1   + Ki*integral_oil1,0),1);

  // Restricción de inventario: no volúmenes negativos
  Qout_total1 = min(apertura_total1*sqrt(Pgas1/1e5)*0.02, max(V_agua1,0));
  Qout_oil1   = min(apertura_oil1*sqrt(Pgas1/1e5)*0.005, max(V_oil1,0));
  ...
  der(integral_total2) = if apertura_total2 < 1 then error_total2 else 0;
  der(integral_oil2)   = if apertura_oil2   < 1 then error_oil2   else 0;

  apertura_total2 = min(max(Kp*error_total2 + Ki*integral_total2,0),1);
  apertura_oil2   = min(max(Kp*error_oil2   + Ki*integral_oil2,0),1);

  Qout_total2 = min(apertura_total2*sqrt(Pgas2/1e5)*0.02, max(V_agua2,0));
  Qout_oil2   = min(apertura_oil2*sqrt(Pgas2/1e5)*0.005, max(V_oil2,0));
  Qout_gas2   = max((Pgas2-1e5)/1e5*0.05,0);
end Separadores_V48_54;
