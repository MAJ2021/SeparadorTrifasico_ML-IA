model SeparadorOil_V46_4
  ...
  Modelica.Blocks.Continuous.PID pid(k=1.0, Ti=300, Td=0.1, y_start=0.5);

equation
  // Balance dinámico con límite de capacidad
  der(V_oil) = if V_oil < V_sep then Qin - Qout else -Qout;

  // Control PID
  pid.u = (0.54*V_sep - V_oil);
  valveOpening = min(max(pid.y, 0), 1);

  // Salida proporcional al volumen disponible
